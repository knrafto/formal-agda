{-# OPTIONS --cubical --no-positivity-check #-}
...
data Ty where
  ...
  [][]T-con : ∀{Γ Δ Σ}{A : Ty Σ}{σ : Subst Γ Δ}{δ : Subst Δ Σ}
    → A [ δ ]T [ σ ]T ≡ A [ δ ∘ σ ]T
...
data Tm  where
  ...
  _[_]t-con : ∀{Γ Δ}{A : Ty Δ} → Tm Δ A → (σ : Subst Γ Δ) → Tm Γ (A [ σ ]T)
